...
#DATOS PARA INSERCION#########Direccion
    ...
    case '4': {#DATOS PARA INSERCION#########pizza
        ...
        break;
    case '5':// codigo para insertar######### Tamaños
            $ningr = $_REQUEST['tamanio'];
            $precio = $_REQUEST['precio'];
            $v = "insert into precio (tamano,precio) values('$ningr',$precio)";
            include('../class/conexion.php');
            $rs = mysql_query($v);
            if (!$rs) {
                echo "El tamaño ya existe!!";
            } else {
                //header("Location:../index/ingrediente.php");
                echo "<script type='text/javascript'>window.parent.location.reload()</script>";
                exit();
