...
<form name="form4" method="post">
    <table width="610px" border="0" align="center">
                    ...
                    <table width="600" cols="3" border="1" cellpadding="0" cellspacing="0" align="center">
                        ...
                        if (isset($_REQUEST['buscar1'])or isset($_REQUEST['criterios'])) {
                            ...
                            if ($buscar1 != "" or $criterios == 'Todos') {
                                if ($criterios == 'Nombre') {
                                    $_pagi_sql = "SELECT * FROM administrador WHERE nombre LIKE '$buscar1%' ORDER BY id_administrador";
                                }
                                if ($criterios == 'Todos') {
                                    $_pagi_sql = "SELECT * FROM administrador ORDER BY id_administrador";
                                }
                                ...
                                while ($row = mysql_fetch_array($result1)) {
                                    echo '<tr><td data-label="id">' . $row["id_administrador"] . '</td><td data-label="nombre">' . $row["nombre"] . '</td>
					    <td data-label="Usuario">' . $row["usuario"] . '</td><td data-label="Correo">' . $row["correo"] . '</td>
                                            <td data-label="Telefono">' . $row["telefono"] . '</td>
                                            <td data-label="Opciones">' .'<a href="../funcion/eliminar.php?idadmin='.$row["id_administrador"].'&fn=1"><img name="Eliminar" src="../img/Eliminar.png"/></a>'. 
                                            '<a href="../funcion/actualizarUsuario.php?idadmin='.$row["id_administrador"].'&fn=1"><img name="Actualizar" src="../img/actu.png"/></a>'.
                                            '</td>'
                                            . '</tr>';
                                }
        ...
        <input type="hidden" name="rfc5" value="<?php echo $idadmin; ?>" />
        </td>
        </tr>
    </table>	
</form>	
